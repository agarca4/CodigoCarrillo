...
public class PresupuestoVehiculosImpl implements Presupuestable<Averiable<Vehiculo, Averia<AveriaVehiculosImpl>>>{
	...
	@Override
	public double calcularPresupuesto(Averiable<Vehiculo, Averia<AveriaVehiculosImpl>> v1) {
		if (((Vehiculo) v1).getAverias() != null) {
			double precioPorHorasTrabajadas = 0;
			double precioPorPiezasUsadas = 0;

			for (Averia<AveriaVehiculosImpl> averia : ((Vehiculo) v1).getAverias()) {
